...
describe.skip('MenuButton A11y', () => {
    ...
    it('should not have any violations', async () => {
        wrapper = await mountReact(<MenuButton label="default" visible popupProps={popupProps}>{menu}</MenuButton>);
        return test(`.${wrapperClassName}`);
    });

    it('should not have any violations when text button', async () => {
        wrapper = await mountReact(<div>
            <MenuButton text label="Document Edit" visible popupProps={popupProps}>{menu}</MenuButton>&nbsp;&nbsp;
        </div>);
        return test(`.${wrapperClassName}`);
    });
});
