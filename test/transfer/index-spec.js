...
describe('Transfer', () => {
    ...
    it('should render by defaultValue', () => {
        ...
        compareDomAndDataSource(wrapper, 1, [
            { label: '1', value: '1' },
        ]);
    ...
    it('should render search box when set showSearch', () => {
        ...
        wrapper = mount(<Transfer showSearch searchPlaceholder="input something..." dataSource={dataSource} />);
    ...
    it('should custom style and text', () => {
        ...
        wrapper = mount(<Transfer className="custom" listStyle={{ width: '200px' }} titles={['Source', 'Target']} operations={['>>', '<<']} dataSource={dataSource} />);

        assert(wrapper.find('div.next-transfer.custom').length === 1);
        assert(findPanel(wrapper, 0).find('ul.next-transfer-panel-list').props().style.width === '200px');
    ...
    it('should support simple mode', () => {
        ...
        assert(findFooter(wrapper, 0).find('a.next-transfer-panel-move-all').text().trim() === '移动全部');
...
function findFooterCount(wrapper, panelIndex) {
    return findFooter(wrapper, panelIndex).find('span.next-transfer-panel-count').text().trim().split(/\s/)[0];
}
