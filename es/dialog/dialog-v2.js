...
var Dialog = function Dialog(props) {
    ...
    var _props$prefix = props.prefix,
        ...
        overflowScroll = _props$overflowScroll === undefined ? true : _props$overflowScroll,
        minMargin = props.minMargin,
        onClose = props.onClose,
        style = props.style,
        others = _objectWithoutProperties(props, ['prefix', 'afterClose', 'hasMask', 'autoFocus', 'className', 'title', 'children', 'footer', 'footerAlign', 'footerActions', 'onOk', 'onCancel', 'okProps', 'cancelProps', 'locale', 'rtl', 'visible', 'closeMode', 'closeIcon', 'animation', 'cache', 'wrapperStyle', 'popupContainer', 'dialogRender', 'centered', 'top', 'bottom', 'width', 'height', 'overflowScroll', 'minMargin', 'onClose', 'style']);
    ...
    closeMode.forEach(function (mode) {
        switch (mode) {
            case 'esc':
                canCloseByEsc = true;
                break;
    ...
    useEffect(function () {
        if (visible && hasMask) {
            ...
            if (dom.hasScroll(document.body)) {
                ...
                if (scrollWidth) {
                    dom.setStyle(document.body, 'paddingRight', dom.getStyle(document.body, 'paddingRight') + dom.scrollbar().width + 'px');
                }
            }
        }
    }, [visible && hasMask]);
    ...
    var handleExiting = function handleExiting() {
        setTimeout(function () {
            return document.body.setAttribute('style', originStyle.current || '');
        }, animation === null ? 0 : 100);
    };
