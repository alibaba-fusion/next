...
var Base = (_temp = _class = function (_React$Component) {
    ...
    Base.prototype.renderMenu = function renderMenu() {
        ...
        return React.createElement(Menu, menuProps);
    };
            ...
            if (Array.isArray(item.children)) {
                return React.createElement(
                    MenuGroup,
                    {
                        key: index,
                        label: item.label },
                    _this6.renderMenuItem(item.children)
                );
            } else {
                var itemProps = {
                    key: item.value,
    ...
    Base.prototype.render = function render(props) {
        ...
        return React.createElement(
            ...
            popupContent ? React.createElement(
                'div',
                { className: prefix + 'select-popup-wrap', style: autoWidth ? { width: this.width } : { minWidth: this.width } },
                popupContent
            ) : this.renderMenu()
        );
    };
