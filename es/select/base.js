...
import { isSingle, filter, isNull, valueToSelectKey, getValueDataSource, generateGroupKey, } from './util';
    ...
    Base.prototype.renderMenuItem = function (dataSource) {
        ...
        return dataSource.map(function (item, index) {
            ...
            if (Array.isArray(item.children) && showDataSourceChildren) {
                var groupKey = generateGroupKey(index, itemValueSet);
                return (React.createElement(MenuGroup, { key: groupKey, label: item.label }, _this.renderMenuItem(item.children)));
            }
            else {
                var itemProps = {
                    role: 'option',
