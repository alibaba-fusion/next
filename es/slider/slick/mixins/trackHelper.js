...
export var getTrackCSS = function (spec) {
    ...
    if (trackWidth) {
        style = __assign(__assign({}, style), { width: trackWidth });
    }
    if (trackHeight) {
        style = __assign(__assign({}, style), { height: trackHeight });
    }
    if (spec.animation !== 'fade') {
        style = __assign(__assign({}, style), transform);
    }
    return style;
};
...
export var getTrackLeft = function (spec) {
    ...
    if (!spec.vertical) {
        ...
        if (spec.adaptiveHeight) {
            ...
            var newHeight = spec.slideHeightList
                .slice(0, index)
