...
var SubNav = (_temp = _class = function (_Component) {
    ...
    SubNav.prototype.render = function render() {
        ...
        if (iconOnly) {
            if (hasArrow) {
                iconEl = React.createElement(Icon, { className: prefix + 'nav-icon-only-arrow', type: mode === 'popup' ? 'arrow-right' : 'arrow-down' });
            } else if (!iconEl) {
                iconEl = React.createElement('span', { className: prefix + 'nav-icon-placeholder' });
            }
        }
