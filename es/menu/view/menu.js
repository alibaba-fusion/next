...
var Menu = (_temp = _class = function (_Component) {
    ...
    Menu.prototype.adjustChildrenWidth = function adjustChildrenWidth() {
        ...
        var spaceWidth = getWidth(this.menuNode);
                ...
                _typeof(child.type) === 'object') && 'menuChildType' in child.type) {
                    ...
                    switch (child.type.menuChildType) {
                        case 'submenu':
                            ...
                            break;
                        case 'group':
                            newChild = cloneElement(child, props, loop(child.props.children, posPrefix, indexWrapper, childLevel));
                            break;
                        case 'item':
                        case 'divider':
    ...
    Menu.prototype.handleOpen = function handleOpen(key, open, triggerType, e) {
        ...
        if (open && index === -1) {
            if (mode === 'inline') {
                if (openMode === 'single') {
                    newOpenKeys = openKeys.filter(function (k) {
                        return !_this5.isSibling(_this5.k2n[key].pos, _this5.k2n[k].pos);
                    });
                ...
                newOpenKeys = openKeys.filter(function (k) {
                    return _this5.isAncestor(_this5.k2n[key].pos, _this5.k2n[k].pos);
                });
                newOpenKeys.push(key);
            }
            ...
            if (mode === 'inline') {
                ...
                newOpenKeys = openKeys.filter(function (k) {
                    return k !== key && !_this5.isAncestor(_this5.k2n[k].pos, _this5.k2n[key].pos);
                });
            }
        }
            ...
            if (item.props.parentMode === 'popup' && this.state.openKeys.length) {
                ...
                this.props.onOpen([], {
                    key: this.state.openKeys.sort(function (prevKey, nextKey) {
                        return _this6.k2n[nextKey].pos.split('-').length - _this6.k2n[prevKey].pos.split('-').length;
                    })[0],
                    open: false
                });
            }
    ...
    Menu.prototype.getAvailableKey = function getAvailableKey(pos, prev) {
        ...
        var ps = Object.keys(this.p2n).filter(function (p) {
            return _this7.isAvailablePos(pos, p);
        });
        if (ps.length > 1) {
    ...
    Menu.prototype.getFirstAvaliablelChildKey = function getFirstAvaliablelChildKey(parentPos) {
        ...
        var pos = Object.keys(this.p2n).find(function (p) {
            return _this8.isAvailablePos(parentPos + '-0', p);
        });
    ...
    this.addIndicators = function (children) {
        var arr = [];
        var lastVisibleIndex = _this9.state.lastVisibleIndex;
        ...
        children.forEach(function (child, index) {
            ...
            if (index === lastVisibleIndex + 1) {
                ...
                arr.push(_this9.getIndicatorsItem(overflowedItems));
            }

            arr.push(child);
        });

        arr.push(_this9.getIndicatorsItem([], true));
