...
export function getAllCheckedKeys(checkedKeys, _k2n, _p2n) {
    ...
    var flatKeys = filterChildKey(filteredKeys, _k2n, _p2n);
    ...
    for (var i = 0; i < keys.length; i++) {
        ...
        for (var j = nums.length - 2; j > 0; j--) {
            ...
            var _parent = _p2n[parentPos];
            if (_parent.checkable === false || _parent.disabled || _parent.checkboxDisabled) continue;
            var parentChecked = _parent.children.every(function (child) {
                return isNodeChecked(child, flatKeys);
            });
            if (parentChecked) {
                _parent.children.forEach(removeKey);
                addParentKey(i, _parent);
            } else {
                break;
            }
        }
