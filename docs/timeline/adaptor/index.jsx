    ...
    adaptor: ({ state, timeLeft, content, height, style, data, ...others }) => {
        ...
        list.forEach((item, index) => {
            ...
            item.children.forEach((it, index) => {
                if (index === 0) {
                    obj.content = it.value.filter(({ type }) => type === ContentType.text).map(({ value }) => value).join('');
                } else if (index === 1) {
                    obj[timeLeft ? 'time' : 'timeLeft'] = it.value.filter(({ type }) => type === ContentType.text).map(({ value }) => value).join('');
                }
            });
            ...
            <Timeline {...others} style={{ minHeight: height, ...style }}>
                {
                    array.map(({ title, icon, content, time, timeLeft, state }, index) => (
                        <Timeline.Item key={index} state={state} title={title} icon={icon} time={time} timeLeft={timeLeft} content={content} />
                    ))
                }
            </Timeline>
        );
