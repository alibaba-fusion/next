...
function renderButton(type, locale, props) {
    const menu = ['undo', 'redo', 'cut', 'copy', 'paste'].map(item => <MenuButton.Item key={item}>{locale[item]}</MenuButton.Item>);
    ...
    return (<Demo title={type} block style={style}>
        ...
        <DemoGroup label={locale.normal}>
            <MenuButton visible={false} size="large" {...commonProps}>{menu}</MenuButton>
            <MenuButton visible={false} size="medium" {...commonProps}>{menu}</MenuButton>
            <MenuButton visible={false} size="small" {...commonProps}>{menu}</MenuButton>
        </DemoGroup>
        <DemoGroup label={locale.expand} height="260px">
            <MenuButton visible={true} size="large" popupProps={{ needAdjust: false }} {...commonProps}>{menu}</MenuButton>
            <MenuButton visible={true} size="medium" popupProps={{ needAdjust: false }} {...commonProps}>{menu}</MenuButton>
            <MenuButton visible={true} size="small" popupProps={{ needAdjust: false }} {...commonProps}>{menu}</MenuButton>
        </DemoGroup>
        <DemoGroup label={locale.disable}>
            <MenuButton disabled size="large" {...commonProps}>{menu}</MenuButton>
            <MenuButton disabled size="medium" {...commonProps}>{menu}</MenuButton>
            <MenuButton disabled size="small" {...commonProps}>{menu}</MenuButton>
        </DemoGroup>
    </Demo>)
}
