...
| formatter       | 数据格式化函数，配合自定义 action 使用，参数为服务器的响应数据，详见 [formatter](#formater)<br><br>**签名**:<br>Function(response: String, file: File) => void<br>**参数**:<br>_response_: {String} 返回<br>_file_: {File} 文件对象                                                   | Function        | -         |
