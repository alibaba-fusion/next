...
| maxTagPlaceholder     | 隐藏多余 tag 时显示的内容，在 maxTagCount 生效时起作用<br><br>**签名**:<br>Function(selected: number, total: number) => void<br>**参数**:<br>_selected_: {number} 当前已选中的元素个数<br>_total_: {number} 总待选元素个数                                                                       | Function                               | -                                  |
