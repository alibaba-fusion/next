...
| onChange                | callback triggered when value change<br><br>**signatures**:<br>Function(value: String/Array, data: Object/Array) => void<br>**params**:<br>_value_: {String/Array} selected value, a single value is returned when single select, and an array is returned when multiple select<br>_data_: {Object/Array} selected data, including value, label, pos, key and rawData properties, returns a single value when single select, returns an array when multiple select, parent and child nodes are selected at the same time, only the parent node is returned. | Function                | () => {}    |
...
| treeLoadData            | asynchronous data loading function, Please refer to Tree's asynchronous loading data Demo<br><br>**signatures**:<br>Function(node: ReactElement) => void<br>**params**:<br>_node_: {ReactElement} clicked node                       | Function                | -           |
