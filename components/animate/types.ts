...
export interface OverlayAnimateProps {
     ...
     * @en By default the child component is mounted immediately along with the
     ...
     * @en By default the child component stays mounted after it reaches the
    ...
    /**
     * 在“进入”状态被应用前触发的回调。我们提供了一个额外的参数 isAppearing，用于标识进入阶段是否在初次挂载时发生。
     * @en Callback fired before the "entering" status is applied. An extra
    ...
    /**
     * 在“进入”状态被应用后触发的回调。我们提供了一个额外的参数 isAppearing，用于标识进入阶段是否在初次挂载时发生。
     * @en Callback fired after the "entering" status is applied. An extra parameter
    ...
    /**
     * 在“已进入”状态被应用后触发的回调。我们提供了一个额外的参数 isAppearing，用于标识进入阶段是否在初次挂载时发生。
     * @en Callback fired after the "entered" status is applied. An extra parameter
     * isAppearing is supplied to indicate if the enter stage is occurring on
     * the initial mount
     * @param node - 节点 - node
     * @param isAppearing - 是否在初次挂载 - isAppearing
     ...
     * @en Normally a component is not transitioned if it is shown when the
