...
function assertTree(props: Pick<TreeProps, 'dataSource' | 'children' | 'immutable'>) {
    cy.mount(
        <Tree
            checkable
            editable
...
function assertExpanded(key: Key, expanded: boolean, childrenKeys?: Key[]) {
...
function assertSelected(key: Key, selected: boolean) {
...
function assertChecked(key: Key, checked: boolean) {
    findTreeNodeByKey(key)
...
function assertIndeterminate(key: Key, indeterminate: boolean) {
    findTreeNodeByKey(key)
        .find('.next-checkbox-wrapper')
        .first()
...
//     mouseClickEvents.forEach(mouseEventType =>z
...
function dragTreeNode(dragKey: Key, dropKey: Key, dropPosition: number, isCanDrop = true) {
    ...
    if (isCanDrop) {
        ...
        // dragNodeLabel.trigger('dragend');
    }
}

function assertEditing(key: Key, editing: boolean) {
...
class DragDemo extends React.Component<any, { gData: ReturnType<typeof cloneData> }> {
    static propTypes = {
...
describe('Tree', () => {
    it('should render by children', () => {
        ...
        assertTree({ children: loop(dataSource), dataSource });
    });
        ...
        cy.mount(
            <Tree
                ...
                renderChildNodes={nodes => {
                    if (nodes.filter(node => !node.props.children).length !== nodes.length) {
                        return (
    ...
    it('should support keyboard', () => {
        ...
        findInnerNodeLabelWrapperByKey('1').then($el => {
            $el[0].focus();
            cy.document().its('activeElement').should('equal', $el[0]);
            cy.document().then(doc => {
                cy.wrap(doc.activeElement).trigger('keydown', {
                    keyCode: KEYCODE.RIGHT,
                });
                findInnerNodeLabelWrapperByKey('2').then($el => {
            ...
            cy.document().then(doc => {
                ...
                findInnerNodeLabelWrapperByKey('3').then($el => {
                    cy.document().its('activeElement').should('eq', $el[0]);
                });
            });
    ...
    it('should support immutable', () => {
        assertTree({ dataSource: freeze(dataSource), immutable: true });
    });
