...
import type { PannelType, PanelProps } from './types';
...
class TimePickerPanel extends Component<PanelProps> {
    ...
    getDisabledItems = () => {
        ...
        disableds.newDisabledHours[0] = (h: number) =>
            (v1 && h > v1.hour()) || (dHours as number[]).indexOf(h) > -1;
        disableds.newDisabledMinutes[0] = (m: number) =>
            (v1 && hoursEqual() && m > v1.minute()) || (dMinutes as number[]).indexOf(m) > -1;
        disableds.newDisabledSeconds[0] = (s: number) =>
            (v1 && minutesEqual() && s > v1.second()) || (dSeconds as number[]).indexOf(s) > -1;

        disableds.newDisabledHours[1] = (h: number) =>
            (v0 && h < v0.hour()) || (dHours as number[]).indexOf(h) > -1;
        disableds.newDisabledMinutes[1] = (m: number) =>
            (v0 && m < ((hoursEqual() && v0.minute()) as number)) ||
            (dMinutes as number[]).indexOf(m) > -1;
        disableds.newDisabledSeconds[1] = (s: number) =>
            (v0 && minutesEqual() && s < v0.second()) || (dSeconds as number[]).indexOf(s) > -1;
            ...
            <React.Fragment>
                {showHour ? (
                    <TimeMenu
                        ...
                        disabledItems={newDisabledHours[index] as (index: number) => boolean}
                    />
                ) : null}
                {showMinute ? (
                    <TimeMenu
                        ...
                        disabledItems={newDisabledMinutes[index] as (index: number) => boolean}
                    />
                ) : null}
                {showSecond ? (
                    <TimeMenu
                        ...
                        disabledItems={newDisabledSeconds[index] as (index: number) => boolean}
                    />
                ) : null}
            </React.Fragment>
        );
