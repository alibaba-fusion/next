...
export interface GroupHeaderProps extends HTMLAttributesWeak, CommonProps {
    ...
    cell?:
        | React.ReactNode
        | ((value: RecordItem, index: number, record?: unknown) => React.ReactNode);
...
export interface TableProps
     ...
     * 点击表格每一行触发的事件
     */
    onRowClick?: (record: unknown, index: number, e: React.MouseEvent) => void;

    /**
     * 悬浮在表格每一行的时候触发的事件
     */
    onRowMouseEnter?: (record: unknown, index: number, e: React.MouseEvent) => void;

    /**
     * 离开表格每一行的时候触发的事件
     */
    onRowMouseLeave?: (record: unknown, index: number, e: React.MouseEvent) => void;
    ...
    cellProps?: (
        ...
        dataIndex: string,
        record: RecordItem
    ) => Partial<CellProps> | undefined | void;
