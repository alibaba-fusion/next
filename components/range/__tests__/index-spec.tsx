...
describe('Range ', () => {
    ...
    it('defaultValue', () => {
        ...
        cy.get<MountReturn>('@el').then(({ component, rerender }) => {
            return rerender(React.cloneElement(component as ReactElement, { defaultValue: 20 }));
        });
        checkRangeSelectedDomWidthAndLeft('10%', '0%');
        ...
        cy.get<MountReturn>('@el').then(({ component, rerender }) => {
            return rerender(
                React.cloneElement(component as ReactElement, { defaultValue: [30, 40] })
            );
        });
        checkRangeSelectedDomWidthAndLeft('20%', '10%');
    });
    it('value', () => {
        ...
        cy.get<MountReturn>('@el').then(({ component, rerender }) => {
            return rerender(React.cloneElement(component as ReactElement, { value: 20 }));
        });
        ...
        cy.get<MountReturn>('@el').then(({ component, rerender }) => {
            return rerender(React.cloneElement(component as ReactElement, { value: [30, 40] }));
        });
    ...
    it('marks', () => {
        ...
        cy.get('.next-range-scale-item').should('to.length', 4);
        ...
        cy.get('.next-range-scale-item').should('to.length', 11);
        ...
        cy.get('.next-range-scale-item').should('to.length', 4);
        cy.get('.next-range-mark-text').first().should('to.have.text', '0°C');
    });
    ...
    it('laptop dragging onChange onProcess', () => {
        ...
        cy.get('@onChange').should('have.been.calledWith', 20);
        cy.get('@onProcess').should('have.been.calledOnce');
        cy.get('@onProcess').should('have.been.calledWith', 20);
    });
    ...
    it('exchange upper and lower', () => {
        ...
        cy.get('@onChange').should('have.been.calledWith', [20, 30]);
        ...
        cy.get('@onChange').should('have.been.calledWith', [0, 20]);
    });
    ...
    it('set value === undefined for reset', () => {
        ...
        cy.get<MountReturn>('@el').then(({ component, rerender }) => {
            return rerender(React.cloneElement(component as ReactElement, { value: undefined }));
        });
    ...
    it('reverse', () => {
        ...
        cy.get('.next-range-selected').should('to.length', 2);
        cy.get('.next-range-selected')
            .eq(0)
            .then($el => {
                expect({
    ...
    it('fixedWidth dragging should have active class', () => {
        ...
        cy.get('.next-range-active').should('to.exist');
    });
