...
export interface RangeProps extends HTMLAttributesWeak, CommonProps {
    ...
    /**
     * 滑块个数
     * @en number of sliders
     * @defaultValue 'single'
     * @remarks \<br\/\>\<br\/\>**可选值**:\<br\/\>'single'(单个)\<br\/\>'double'(两个) - \<br\>\<br\>**option**:\<br\>'single'\<br\>'double'
     */
    ...
    /**
     * 步长
     * @en step of the range
     * @defaultValue 1
     * @remarks 取值必须大于 0，并且可被 (max - min) 整除 - which is positive integer and (max - min) can be divided by it
     */
    step?: number;
     ...
     * 设置当前取值
     * @en current value
     * @remarks 当 `slider` 为 `single` 时，使用 `Number`，否则用 `[Number, Number]` - It's in the form of `Number` when `slider` is `single` otherwise `[Number, Number]`
     */
    ...
    /**
     * 刻度数值显示逻辑
     * @en way to show the scale
     * @defaultValue false
     * @remarks false 代表不显示，array 枚举显示的值，number 代表按 number 平分，object 表示按 key 划分，value 值显示 - false means nothing, array means enum, number means equal division, and object means key as the mark with value as the value
     */
    marks?: false | number | Array<number> | Record<number, string>;

    /**
     * marks显示在上方('above')or下方('below')
     * @en position for the scale
     * @defaultValue 'above'
     * @remarks 上方('above')or下方('below') - 'above' means above, 'below' means below
     */
     ...
     * @defaultValue () =\> void
     * @remarks 当 Range 的值发生改变后，会触发 onChange 事件，并把改变后的值作为参数传入, 如果设置了value, 要配合此函数做受控使用
     */
     ...
     * @param value - 改变后的值 - the changed value
     * @defaultValue () =\> void
     * @remarks 不建议在这里setState, 一般情况下不需要用, 滑动时有特殊需求时使用
     */
    ...
    /**
     * 是否为拖动线段类型
     * @en drag a line with fixed width
     * @defaultValue false
     * @remarks 默认slider为double, defaultValue必传且指定区间 - It considers `slider` as `double`, and `defaultValue` must be a interval.
     */
    fixedWidth?: boolean;
     ...
     * @returns React.ReactNode - React.ReactNode
     */
    renderPreview?: (value: RangeValueType, props: RangeProps) => React.ReactNode;
}

export interface RangeMarkProps
    extends Pick<Required<RangeProps>, 'min' | 'max' | 'prefix' | 'marksPosition' | 'rtl'> {
    value: RangeValueType;
...
export interface RangeScaleProps
    extends Pick<Required<RangeProps>, 'min' | 'max' | 'prefix' | 'rtl'> {
    value: RangeValueType;
    scales: number[];
}

export interface RangeSelectedProps
    extends Pick<Required<RangeProps>, 'min' | 'max' | 'prefix' | 'slider' | 'reverse' | 'rtl'> {
    value: RangeValueType;
}

export interface RangeFixedSliderProps
    extends Pick<
        Required<RangeProps>,
        'min' | 'max' | 'prefix' | 'hasTip' | 'tooltipVisible' | 'tipRender' | 'disabled' | 'rtl'
    > {
    value: [number, number];
