...
var Base = /** @class */ (function (_super) {
    ...
    Base.prototype.renderMenuItem = function (dataSource) {
        var e_1, _a;
        var _this = this;
        var _b = this.props, prefix = _b.prefix, itemRender = _b.itemRender, showDataSourceChildren = _b.showDataSourceChildren;
        // If it has.
        var searchKey;
        ...
        var itemValueSet = new Set();
        try {
            for (var dataSource_1 = tslib_1.__values(dataSource), dataSource_1_1 = dataSource_1.next(); !dataSource_1_1.done; dataSource_1_1 = dataSource_1.next()) {
                var item = dataSource_1_1.value;
                if (item && !(Array.isArray(item.children) && showDataSourceChildren)) {
                    itemValueSet.add("".concat(item.value));
                }
            }
        }
        ...
        return dataSource.map(function (item, index) {
            ...
            if (Array.isArray(item.children) && showDataSourceChildren) {
                var groupKey = (0, util_2.generateGroupKey)(index, itemValueSet);
                return (react_1.default.createElement(MenuGroup, { key: groupKey, label: item.label }, _this.renderMenuItem(item.children)));
            }
            else {
                var itemProps = {
                    role: 'option',
