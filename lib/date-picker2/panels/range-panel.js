...
var getPanelValue = function getPanelValue(_ref, oldPanelValue) {
    var mode = _ref.mode,
        inputType = _ref.inputType,
        value = _ref.value,
        showTime = _ref.showTime;

    var panelValue = (0, _util.datejs)(oldPanelValue);
...
var RangePanel = (_temp = _class = function (_React$Component) {
    ...
    function RangePanel(props) {
        ...
        var timeValue = value && [].concat(value) || timePanelProps.defaultValue || [];
    ...
    value: PT.arrayOf(_propTypes3.default.date),
    ...
    this.onTimeSelect = function (v) {
        ...
        if (!curDateVal) {
            curDateVal = inputType === BEGIN && end ? end.subtract(1, 'day') : inputType === END && begin ? begin.add(1, 'day') : (0, _util.datejs)();
        }
    ...
    this.handleSelect = function (v, fromTimeChange) {
        ...
        var timeVal = fromTimeChange || resetTime ? null : value[inputType] || value[otherType] || (0, _util.datejs)();

        if (inputType === BEGIN) {
            begin = (0, _util2.setTime)(v, timeVal);
            if (end && end.isBefore(begin)) {
                end = null;
            }
        } else {
            end = (0, _util2.setTime)(v, timeVal);
            if (begin && begin.isAfter(end)) {
                begin = null;
            }
        }

        _util.func.invoke(_this2.props, 'onSelect', [[begin, end]]);
    };
    ...
    this.getCellClassName = function (value) {
        ...
        if (!_this2.hasModeChanged) {
            ...
            rangeClassName = mode === WEEK ? (_ref2 = {}, _ref2[prefixCls + '-week-range-begin'] = state === SELECTED_BEGIN, _ref2[prefixCls + '-week-range-end'] = state === SELECTED_END, _ref2) : (_ref3 = {}, _ref3[prefixCls + '-range-begin'] = state === SELECTED_BEGIN, _ref3[prefixCls + '-range-end'] = state === SELECTED_END, _ref3[prefixCls + '-range-begin-single'] = state >= SELECTED && (!end || end.isSame(begin, unit)), _ref3[prefixCls + '-range-end-single'] = state >= SELECTED && (!begin || begin.isSame(end, unit)), _ref3[prefixCls + '-edge-begin'] = edgeState === 1, _ref3[prefixCls + '-edge-end'] = edgeState === 2, _ref3);
        }
