...
type HTMLAttributesWeak<T> = Omit<React.HTMLAttributes<T>, 'content' | 'onClick' | 'title' | 'direction'>;
/**
 * @api
 */
 ...
 * @api Step.Item
 */
export interface ItemProps<T = HTMLElement> extends HTMLAttributesWeak<T>, CommonProps, ItemPrivateProps {
    /**
     ...
     */
    itemRender?: (index: number, status: StepStatus, title?: React.ReactNode, content?: React.ReactNode) => React.ReactNode;
    /**
     * 百分比
     * @en Percent
     */
 ...
 * @api Step
 */
export interface StepProps extends Omit<React.HTMLAttributes<HTMLElement>, 'type' | 'direction'>, CommonProps {
    /**
     * 当前步骤
     * @en Current step
     * @defaultValue 0
     ...
     * @returns 节点的渲染结果 - render content of the node
     */
    itemRender?: (index: number, status: StepStatus, title?: ReactNode, content?: ReactNode) => React.ReactNode;
    /**
     * 宽度是否横向拉伸
