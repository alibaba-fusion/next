    ...
    var LockTable = (_temp = _class = function (_React$Component) {
        ...
        LockTable.prototype.adjustCellSize = function adjustCellSize() {
            ...
            if (this.isLock()) {
                this.tableInc.props.dataSource.forEach(function (item, index) {
                    ...
                    var rowIndex = (typeof item === 'undefined' ? 'undefined' : (0, _typeof3.default)(item)) === 'object' && '__rowIndex' in item ? item.__rowIndex : index;

                    var _tableInc$props$lengt = _this4.tableInc.props.lengths,
                        left = _tableInc$props$lengt.left,
                        right = _tableInc$props$lengt.right,
                        origin = _tableInc$props$lengt.origin;

                    // 同步最左侧的锁列

                    for (var i = 0; i < left; i++) {
                        _this4.setCellSize(rowIndex, i, 'left');
                    }
                    // 同步最右侧的锁列
                    for (var _i = origin - right; _i < origin; _i++) {
                        _this4.setCellSize(rowIndex, _i, 'right');
                    }
                });
            }
        };

        LockTable.prototype.setCellSize = function setCellSize(index, i, dir) {
            var lockRow = this.getCellNode(index, i, dir),
                row = this.getCellNode(index, i),
                rowHeight = (isIE ? row && row.offsetHeight : row && parseFloat(getComputedStyle(row).height)) || 0,
                lockHeight = (isIE ? lockRow && lockRow.offsetHeight : lockRow && parseFloat(getComputedStyle(lockRow).height)) || 0;
        ...
        LockTable.prototype.render = function render() {
            ...
            var _props2 = this.props,
                ...
                others = (0, _objectWithoutProperties3.default)(_props2, ['children', 'prefix', 'components', 'className', 'dataSource']);
