...
const { logger, getComponentName, CWD, SRC_DIR_PATH } = require('../../utils');
...
Object.keys(parentMap).forEach(child => {
    ...
    parentPathMap[child] = {
        name: parent,
        path: path.join(SRC_DIR_PATH, component),
    };
});
...
module.exports = function(options) {
    ...
    options.entries.forEach(entry => {
        ...
        const srcComponentPath = path.join(SRC_DIR_PATH, shortName);
...
`
    ...
    const localePaths = glob.sync(path.join(SRC_DIR_PATH, 'locale/**.*'));
