...
if (orderReg.test(varContent)) {
  newVarContent = varContent.replace(orderReg, function() {
    return arguments[1] + JSON.stringify(order);
  });
} else {
  const metaReg = /(\/\/\/\/(\s|.)+?)(\/\/\/\/)/;
  newVarContent = varContent.replace(metaReg, function() {
    return arguments[1] + `/// @order ${JSON.stringify(order)}\n` + arguments[3];
  });
}
