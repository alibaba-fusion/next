...
module.exports = function getWebpackConfig(options) {
    ...
    config.output = {
        path: `${cwd}/__html/rtl/${componentName}`,
        publicPath: '/',
        filename: '[name].js'
    };
    ...
    if (themePaths.length) {
        ...
        if (componentName === 'core') {
            ...
            links.push({
                href: `rtl/${path.relative(docsPath, themePaths[0]).replace(/\.jsx$/, '.html')}`,
                title: 'Theme Demo'
            });
            links.push({
                href: `rtl/${path.relative(docsPath, themePaths[0]).replace(/index\.jsx$/, 'config.html')}`,
...
function getEntry(entryPaths, componentName) {
    const entry = entryPaths.reduce((ret, entryPath) => {
        ...
        pathWithoutExt = pathWithoutExt.replace(path.join(cwd, 'docs', componentName), '');
        ret[pathWithoutExt] = [
...
function getLinks(demoPaths) {
    ...
    return orderedDemoPaths.map(demoPath => {
        const href = `rtl/${path.relative(path.join(cwd, 'docs'), demoPath).replace(/\.md$/, '.html')}`;
